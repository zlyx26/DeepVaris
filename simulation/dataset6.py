...
def main():
    ...
    Lambda_list=[10,6,3,1.5,0.7]
    simulation_iterations = 10
    num_epochs_cnn = 50
    num_epochs_mask_list = [10,10,10,20,30]
    num_epochs_dnn = 10
    ...
    Epochs = 50
    dataset_file_name = f'dataset6_{samples_num}_{set_feature_num}'
    for Epoch in range(Epochs):
        ...
        if not os.path.exists(
            ...
            os.makedirs(
                f'results/{dataset_file_name}/{dataset_file_name}_{Epoch}_results/{dataset_file_name}_temp_{Epoch}/')
        train_X, train_Y, val_X, val_Y, test_X, test_Y, train_dataloader, val_dataloader, test_dataloader, train_samples_num, val_samples_num, test_samples_num, art, Mask_art, p0, feature_num, non_constant_features = generate_dataset_6(
        ...
        for i in range(len(feature_index)):
            ...
            if select_feature_num == 0:
                sr_rs(Epoch, dataset_name, cnn_initial_loss_p, dnn_initial_loss_p, cnn_initial_correlation, dnn_initial_correlation,
                      999, 999, 0, 0, cnn_initial_loss_p,
                      cnn_initial_correlation, 0, 0, 0,[0])
            else:
