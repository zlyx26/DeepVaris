...
def main():
    ...
    num_classes = 2
    num_block = 1
    Lambda_list = [0.05,0.01,0.005,0.001,0.0005]

    simulation_iterations = 2
    num_epochs_cnn = 1
    num_epochs_mask_list = [10,15,15,20,40]
    ...
    for Epoch in range(Epochs):
        ...
        if not os.path.exists(
            ...
            os.makedirs(
                f'results/{dataset_file_name}/{dataset_file_name}_{Epoch}_results/{dataset_file_name}_temp_{Epoch}/')
        train_X, train_Y, val_X, val_Y, test_X, test_Y, train_dataloader, val_dataloader, test_dataloader, train_samples_num, val_samples_num, test_samples_num, art, Mask_art, p0, feature_num, non_constant_features = generate_dataset_2(
             batch_size, k, important_feature_num, True)
