...
def main():
    ...
    Lambda_list =[0.5,0.1,0.05,0.01,0.005]
    simulation_iterations = 3
    #num_epochs_cnn = 1
    num_epochs_cnn = 5
    #num_epochs_mask_list = [10,10,15,30,40]
    num_epochs_mask_list = [20, 30, 40, 40,60]
    num_epochs_dnn = 10
    learning_rate_cnn = 0.00005
    ...
    Epochs = 50
    dataset_file_name=f'dataset3_{samples_num}_{set_feature_num}'
    for Epoch in range(Epochs):
        ...
        if not os.path.exists(
            ...
            os.makedirs(
                f'results/{dataset_file_name}/{dataset_file_name}_{Epoch}_results/{dataset_file_name}_temp_{Epoch}/')
        train_X, train_Y, val_X, val_Y, test_X, test_Y, train_dataloader, val_dataloader, test_dataloader, train_samples_num, val_samples_num, test_samples_num, art, Mask_art, p0, feature_num, non_constant_features = generate_dataset_3(
        ...
        for i in range(simulation_iterations):
            ...
            for j in range(len(Lambda_list)):
                ...
                cleanup()
                Mask_net = Mask_Network2(num_block, feature_num, k)
