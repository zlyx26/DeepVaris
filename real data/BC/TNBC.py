...
def main():
    ...
    k=5
    batch_size = 32
    dataset_file_name='tnbc'
    num_classes = 2
    num_block = 1
    Lambda_list = [ 0.05,0.03,0.01,0.005,0.003,0.001,0.0005]
    num_epochs_cnn = 5
    num_epochs_mask_list = [20,20,30,30,50,50,50]
    num_epochs_dnn=10
    ...
    for Epoch in range(Epochs):
        ...
        if not os.path.exists(
            ...
            os.makedirs(
                f'results/{dataset_file_name}/{dataset_file_name}_{Epoch}_results/{dataset_file_name}_temp_{Epoch}/')
        train_X, train_Y, val_X, val_Y, test_X, test_Y, train_dataloader, val_dataloader, test_dataloader, train_samples_num, val_samples_num, test_samples_num, p0, feature_num, non_constant_features = load_tnbc(
            batch_size, k, True)
