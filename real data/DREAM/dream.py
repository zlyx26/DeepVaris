...
def main():
    ...
    k = 5
    batch_size = 32
    dataset_file_name='dream'
    num_classes = 2
    num_block = 1
    Lambda_list = [0.01, 0.005,0.001,0.0005,0.0001,0.00005]

    num_epochs_cnn = 50
    num_epochs_mask_list = [150, 300, 350, 300, 450, 500]
    num_epochs_dnn=10
    ...
    for Epoch in range(Epochs):
        ...
        if not os.path.exists(
            ...
            os.makedirs(
                f'results/{dataset_file_name}/{dataset_file_name}_{Epoch}_results/{dataset_file_name}_temp_{Epoch}/')
        train_X, train_Y, val_X, val_Y, test_X, test_Y, train_dataloader, val_dataloader, test_dataloader, train_samples_num, val_samples_num, test_samples_num, p0, feature_num, non_constant_features = load_dream(
            batch_size, k,True)
        ...
        for i in range(simulation_iterations):
            ...
            for j in range(len(Lambda_list)):
                ...
                if select_feature_num != 0:
                    train_dataloader_dnn, val_dataloader_dnn, test_dataloader_dnn = get_mask_data(train_X, train_Y,val_X,val_Y,
                                                                                                  test_X, test_Y,select_feature_index.flatten().tolist())
                    cleanup()
